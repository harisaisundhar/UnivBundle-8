...
def initialize_particles():
    for i in range(MAX_PARTICLES):
        ...
        for j in range(MAX_INPUTS):
            newParticle.set_data(j, random.randrange(
...
def get_velocity(gBestindex):
    ...
    for i in range(MAX_PARTICLES):
        ...
        vValue = particles[i].get_velocity() + 2 * random.random() * (
            particles[i].get_pBest() - testResults) + 2 * random.random() * (bestResults - testResults)
    ...
    for i in range(MAX_PARTICLES):
        for j in range(MAX_INPUTS):
            if particles[i].get_data(j) != particles[gBestindex].get_data(j):
                particles[i].set_data(j, particles[i].get_data(
                    j) + math.floor(particles[i].get_velocity()))
    ...
    if theTarget < len(particles):
        ...
        for i in range(MAX_INPUTS):
            if i < MAX_INPUTS - 1:
                ...
                sys.stdout.write(
                    str(particles[theTarget].get_data(i)) + " = " + str(TARGET))
